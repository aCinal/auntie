...
This implementation deals with Ironwood bundles exclusively. The operator receives the settlement as a fully authorized Ironwood bundle and is expected to embed it in a shielded transaction with neither a transparent bundle nor a Sapling bundle. The operator cannot cheat as the Ironwood bundle produced inside the TEE already commits to this transaction (see `src/common/trusted/rust/zcash_ffi/src/digests.rs`).
